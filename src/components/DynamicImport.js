...
const DynamicImport = (component, callback, script_name) => {
  ...
  const allPreviousLoaded = Object.keys(scripts)
      .every((k) => { return scripts[k] === true });
  ...
  const allLoaded = Object.keys(scripts)
      .every((k) => { return scripts[k] === true });
  let loading_message = '';

  if (allPreviousLoaded) {
    loading_message = message.loading('Loading content...', 0);
  }
